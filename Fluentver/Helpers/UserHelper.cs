...
namespace Fluver.Helpers
{
    ...
    {
        private const string AccountPictures = SystemHelper.HKLM + @"SOFTWARE\Microsoft\Windows\CurrentVersion\AccountPicture\Users\";
        private const string StoredIdentities = @".DEFAULT\Software\Microsoft\IdentityCRL\StoredIdentities";

        private const string SIDStart = "S-1-5-21";
        private const char BuiltInEnd = '5';

        private static readonly SecurityIdentifier s_currentSID = WindowsIdentity.GetCurrent().Owner;

        private static readonly Dictionary<string, string> s_personalEmails;

        private static Dictionary<SecurityIdentifier, UserPrincipal> s_users;
        ...
        {
            await CheckUsersAsync();
            return [.. s_users.Values.Where(i => i.Sid != s_currentSID)];
        }
